...
(function () {
  ...
  // Nav data is needed on every page; the rest is page-specific.
  var keys = ['nav'];
  ...
  Promise.all(jobs).then(function (results) {
    ...
    // Map loaded JSON onto the globals the render functions expect.
    window.NAV_WORKSHOPS = (data.nav && data.nav.workshops) || [];
    window.NAV_TEACHING  = (data.nav && data.nav.teaching)  || [];
